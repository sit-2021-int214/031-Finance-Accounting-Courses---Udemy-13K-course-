...
**Created by Phiraphat Kakerd (ID: 63130500088)**
...
```
### 2. What is the total sales in each state? And which state has the least sales?
...
### 3. Find the number of product names (no duplicates)
ใช้ `summarise()` ในการหาจำนวนของ product name ทั้งหมด โดยใช้ `n_distinct()` ในการหาจำนวนของ product name ทีไม่ซ้ำ
```
superstore %>% summarise(CountOfProduct = n_distinct(Product.Name))
...
Summary:
```
1849 This is the total number of products in the superstore.
```

### 4. Find the total of sales for each year.

`mutate()` เพื่อสร้าง column ใหม่ออกมาเป็น coloumn year และใช้ `lubridate` ในการ extract ค่า year ออกมาจาก date โดยมีการตรวจสอบ format ของ Date ว่าเป็นรูปแบบ day/month/year 
คำสั่งที่ใช้ตรวจสอบคือ `year(), dmy()` จากนั้นจึงใช้คำสั่ง `group_by` กับปีที่เรา extract ออกมา และใช้ `summarise` เพื่อหาค่า sum จาก Sales
```
superstore %>% mutate(year=year(dmy(Order.Date)))%>%group_by(year)%>% summarise(Sales =sum(Sales))
...
Summary:
```
 In 2015, the sum of sales amount was 479856.
 In 2016, the sum of sales amount was 459436.
 In 2017, the sum of sales amount was 459436.
 In 2018, the sum of sales amount was 722052.
```
### 5. หาจำนวนการใช้บริการของลูกค้าแต่ละการบริการ(ship Mode)
ใช้คำสั่ง `group_by()` จัดกลุ่มด้วย ship mode และใช้คำสั่ง `count()` เพื่อหาจำนวนของการบริการแต่ละบริการที่ลูกค้าใช้  
```
superstore%>% group_by(Ship.Mode) %>% count(Ship.Mode)
...
Summary:
This is a number of service in each ship mode.
...
### 6. List the name of the product and the number of times the customer made the purchase.
ใช้คำสั่ง `group_by()` จัดกลุ่มด้วย Product name จากนั้นใช้คำสั่ง `tally()` เพื่อหาจำนวนครั้งที่ลูกค้าได้ทำการซื้อสินค้าแต่ละชนิด 
```
superstore%>% group_by(Product.Name)%>%tally()%>% print(n = 20)
...
```
 # A tibble: 1,849 x 2
   Product.Name                                                                                     n
   <chr>                                                                                        <int>
 1 "\"While you Were Out\" Message Book, One Form per Page"                                         3
 2 "#10- 4 1/8\" x 9 1/2\" Recycled Envelopes"                                                     10
 3 "#10- 4 1/8\" x 9 1/2\" Security-Tint Envelopes"                                                 8
 4 "#10-4 1/8\" x 9 1/2\" Premium Diagonal Seam Envelopes"                                          2
 5 "#10 Gummed Flap White Envelopes, 100/Box"                                                       4
 6 "#10 Self-Seal White Envelopes"                                                                  4
 7 "#10 White Business Envelopes,4 1/8 x 9 1/2"                                                     6
 8 "#6 3/4 Gummed Flap White Envelopes"                                                             4
 9 "1.7 Cubic Foot Compact \"Cube\" Office Refrigerators"                                           6
10 "1/4 Fold Party Design Invitations & White Envelopes, 24 8-1/2\" X 11\" Cards, 25 Env./Pack"     2
11 "12-1/2 Diameter Round Wall Clock"                                                               8
12 "12 Colored Short Pencils"                                                                       3
13 "14-7/8 x 11 Blue Bar Computer Printout Paper"                                                   4
14 "2300 Heavy-Duty Transfer File Systems by Perma"                                                 4
15 "24-Hour Round Wall Clock"                                                                       6
16 "24 Capacity Maxi Data Binder Racks, Pearl"                                                      4
17 "3-ring staple pack"                                                                             6
18 "3.6 Cubic Foot Counter Height Office Refrigerator"                                              5
19 "36X48 HARDFLOOR CHAIRMAT"                                                                       6
20 "3D Systems Cube Printer, 2nd Generation, Magenta"                                               2
# ... with 1,829 more rows

```
Summary:
```
The left side is the unique product name and the right side is the number of times the customer has placed an order.
```

## Part 4: Visualization with GGplot2
### 1.) The line chart represents the sales each years.
```
product_year <- superstore  %>% mutate(year = year(dmy(Order.Date))) %>% group_by(year) %>% summarise(sum = sum(Sales))
##ggplot2
product_year_plot <- product_year %>% ggplot(aes(x = year, y = sum)) + geom_line(size = 2, linetype ="dashed", color = "red") + geom_point(color = "blue" , size = 4)

product_year_plot + ggtitle(" sales for each year") + xlab("Years") + ylab("Sales")

```
Result:

![Graph 1](https://github.com/sit-2021-int214/031-Finance-Accounting-Courses---Udemy-13K-course-/blob/main/HW04/HW04_63130500088/graph1.jpg)

จะเห็นได้ว่ายอดขายรวมของปี 2015 จะมีการลดลงไปจนถึงในปี 2016 และจะมียอดขายมากขึ้นไปเรื่อยๆหลังจากนั้น
### 2.) The bar chart represents the number of product each sub-category.
```
Superstore_category<-superstore %>% group_by(Segment) %>% summarise(Total_sales = sum(Sales))
##ggplot2
Superstore_category %>% ggplot(aes(x =Segment , y =Total_sales)) + geom_bar(stat="identity", fill="steelblue") +
  geom_text(aes(label=Total_sales), vjust=1.6, color = "white", size = 3.5)
  ```
Result:

![Graph 2](https://github.com/sit-2021-int214/031-Finance-Accounting-Courses---Udemy-13K-course-/blob/main/HW04/HW04_63130500088/graph2.jpg)

กราฟแสดงจำนวนที่ขายให้กับลูกค้าแต่ละ segment
