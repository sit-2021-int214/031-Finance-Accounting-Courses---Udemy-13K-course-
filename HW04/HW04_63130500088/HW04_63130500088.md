...
ใช้คำสั่ง `group_by()` จัดกลุ่มด้วย ship mode และใช้คำสั่ง `summarise()` เพื่อหาจำนวนของการบริการแต่ละบริการที่ลูกค้าใช้ ใช้คำสั่ง`count()`  
```
library(plyr)
superstore%>% group_by(Ship.Mode) %>% summarise(totalOfShipMode = count(Ship.Mode))
```

Result:

```
  totalOfShipMode.x totalOfShipMode.freq
1       First Class                 1501
2          Same Day                  538
3      Second Class                 1902
